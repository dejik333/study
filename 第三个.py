...
def main():
    ...
    原始文件 = openpyxl.load_workbook(r'%s\深圳原始表格.xlsx' % 桌面路径, data_only=True)
    原始表 = 原始文件['个人费用']
    最大行数 = 原始表.max_row
    ...
    for 行数 in range(2, 最大行数 + 1):
        最终sheet.cell(row=行数, column=column_index_from_string('F')).value = '北京外企深圳德科'
    ...
    for E in range(2, 最大行数+1):
        ...
        N = 原始表.cell(row=E, column=column_index_from_string('DA')).value
        P = 原始表.cell(row=E, column=column_index_from_string('DB')).value

        最终sheet.cell(row=E, column=column_index_from_string('I')).value = 得数值(I) + 得数值(M) + 得数值(N) + 得数值(P)
